...
def train(opt, train_loader, test_loader, tocg, generator, discriminator, model):
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        with torch.no_grad():
            ...
            warped_cloth_paired_taco = F.grid_sample(torch.cat((warped_cloth_tvob, torch.zeros_like(warped_cloth_tvob) ), dim=2), flow_taco_norm + grid_3d, padding_mode='border')
        ...
        if (step+1) % 100 == 0:
            a_0 = im [0]
            b_0 = output [0]
            c_0 = warped_cloth_paired_taco [0]
