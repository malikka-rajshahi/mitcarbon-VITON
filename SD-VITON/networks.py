...
class ConditionGenerator(nn.Module):
    ...
    def forward(self, input1, input2, upsample='bilinear'):
        ...
        for i in range(self.num_layers):
            ...
            if i == 0:
                ...
                if i == layers_max_idx:
                    ...
                    flow_out = self.residual_sequential_flow_list[0](torch.cat((input_3d_flow_out, torch.zeros_like(input_3d_flow_out)  ), dim=2)).permute(0,2,3,4,1)
        ...
        warped_input1_taco = F.grid_sample(torch.cat((warped_input1_tvob, torch.zeros_like(warped_input1_tvob)  ), dim=2), flow_taco_norm + grid_3d, padding_mode='border')
...
def make_grid_3d(N, iH, iW):
    ...
    grid = torch.cat([grid_x, grid_y, grid_z], 4)  
    return grid
