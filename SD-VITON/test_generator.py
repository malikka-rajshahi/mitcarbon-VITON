...
def test(opt, test_loader, board, tocg, generator):
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            print(f'pre_clothes_mask: {pre_clothes_mask.size()}')
            print(f'parse: {label.size()}')
            print(f'parse agnostice: {parse_agnostic.size()}')
            print(f'image agnostic: {agnostic.size()}')
            print(f'opt values: {opt.cond_G_input_height, opt.cond_G_input_width}')
            ...
            warped_cm_onehot = torch.FloatTensor((warped_clothmask_taco.detach().cpu().numpy() > 0.5).astype(np.float64)) 
            ...
            warped_cloth_taco = F.grid_sample(torch.cat((warped_cloth_tvob, torch.zeros_like(warped_cloth_tvob) ), dim=2), flow_taco_norm + grid_3d, padding_mode='border')
            warped_cloth_taco = warped_cloth_taco[:,:,0,:,:]

            warped_clothmask_tvob = warped_clothmask_tvob.unsqueeze(2)
            warped_clothmask_taco = F.grid_sample(torch.cat((warped_clothmask_tvob, torch.zeros_like(warped_clothmask_tvob) ), dim=2), flow_taco_norm + grid_3d, padding_mode='border')
